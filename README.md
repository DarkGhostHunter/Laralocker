...
For example, let's say we need to create a sequential serial key for a sold Ticket. This introduces two problems:
 
* If all jobs started at the same time, these would check the last sold also at the same time, and save the next Ticket with the same serial key.
...
* If we have one Queue Worker, it will only process one Ticket at a time. It will work for a small queue, but if a flood of users buy 1000 tickets in one minute, a single Queue Worker will take its time to process all. 

Using this, all Tickets are dispatched concurrently without fear of collisions, just by announcing what ticket are processing.

## How it works

This package listens for every `Locakble` Job you send to the queue. Then, using your application cache, the Job *look ahead* for free "slots", and reserve it.
...
Add the `Lockable` interface to your Job, Notification or Listener, that is being pushed to a queue (even if is in-sync).

Then, implement the `startFrom()` and `next($slot)` methods, and the `$slot` property empty, in your Job, Notification or Listener. 
...
{
    /**
     * Slot as Serial Key for each Ticket
     *
     * @var string
     */
    public $slot;
    ...
    public function handle(TicketSold $event)
    {
        ...
        // Finally save the Ticket into the system
        $ticket->save();
    }
}
```
...
Once this starting point is retrieved, the Locker will save the last used in the Cache instead of executing this method. This is used only when the first Job hits the queue, or if the cache returns null (maybe because you flushed it).
...
If the next slot is "reserved" in the Cache by another Job, it will recursively call `next($slot)` until it finds one free.
...
Also entirely optional. Slots are reserved by a given time by using the Cache. While the default is of 60 seconds, you can set bigger if your Job takes its sweet time, like 3 minutes or more.
...
Before *handling* the Job, a custom Event Listener will check if it has the `Lockable` interface. If that's the case, then it will pass the Job to the Locker, that will checks what was the last slot used for the Job using the Cache.
